import Btp from "../models/BtpModel.js";
import path from 'path';
import fs from 'fs';

export const getContents = async(req, res) => {
   try {
        const response = await Btp.findAll();
        //console.log()
        res.set({
...
export const getContentById = async(req, res) => {
    try {
        const response = await Btp.findOne({
            where:{
                id: req.params.id
            }
        });
...
export const saveContent = (req, res) => {
    ...
    file.mv(`./public/images/${fileName}`, async(err)=>{
        ...
        try {
            await Btp.create({name: name, image: fileName, url: url});
...
export const updateContent = async(req, res) => {
    const contenu = await Btp.findOne({
        where:{
            id: req.params.id
        }
    });
